...
/*!
 * this function initializes the selected mcg mode
 * returns the value of the configured clock frequency
 */
uint32_t init_mcg(uint8_t mcg_init, uint32_t IRC_freq, uint8_t IRC)
{
	...
	{
		case FBI:
			mcg_clk_hz = fei_fbi(IRC_freq, IRC);// 64 Hz ---> 32768
		break;
		case FBE:
			mcg_clk_hz = fei_fbi(IRC_freq, IRC);// 64 Hz ---> 32768
...
// check if in FBI mode
  if (!((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x1) && // check CLKS mux has selcted uint32_t ref clk
      (MCG->S & MCG_S_IREFST_MASK) &&                                  // check FLL ref is internal ref clk
      (!(MCG->S & MCG_S_PLLST_MASK)) &&                                // check PLLS mux has selected FLL
...
uint32_t fei_fee(uint32_t crystal_val, uint8_t hgo_val, uint8_t erefs_val)
{
  ...
  uint8_t temp_reg;
 // int16_t i;
  uint32_t mcg_out, fll_ref_freq, i;
...
{
  uint8_t disable_rtc_clk_gate = 0;
  uint8_t disable_rtc_tce = 0;
  uint8_t temp_reg;
...
// Set up autocal registers, must use floating point calculation
  if (irc_select)
    atcv = (uint16_t)(128.0f * (21.0f * (bus_clock_freq / (mcg_float_t)irc_freq)));
  else
    atcv = (uint16_t)(21.0f * (bus_clock_freq / (mcg_float_t)irc_freq));
...
uint8_t what_mcg_mode(void)
{
  ...
  // check if in BLPI mode
  else if ((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x1) && // check CLKS mux has selcted uint32_t ref clk
          (MCG->S & MCG_S_IREFST_MASK) &&                                 // check FLL ref is internal ref clk
          (!(MCG->S & MCG_S_PLLST_MASK)) &&                               // check PLLS mux has selected FLL
          (MCG->C2 & MCG_C2_LP_MASK))                                     // check LP bit is set
  {
